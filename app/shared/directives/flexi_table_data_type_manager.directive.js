myApp.directive("dataTypeManager", [function () {
    return {
        restrict: 'A',
        scope: {
            cellConf: "=",
        ...
        replace: false,
        templateUrl: "app/shared/directives/flexi_table_data_type_manager.directive.html",
        link: function (scope, elem) {

            scope.customEval = function(exp){
                if (exp){
                    var res = eval(exp);
                    return res;
                }
            ...
            scope.eventManager = function(column, data){
                var params = {
                    record: data,
                    field_name: {
                        key: column.field_name,
                        value: data[column.field_name]
                    }
                };
                angular.forEach(column.event.params, function(obj,index){
                    if (obj.indexOf(':') > -1){
                ...
                scope.$eval(column.generatedEvent(params));
                // cellConf.generatedEvent(cellConf.event.params);
            }
