HRTabsColController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService","$http","ngDialog","toaster","$state"];
...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
 ...
 $stateProvider	  
	.state('app.hr_tabs_col', {
        url: '/hr_tabs_col',
        title: 'Human Resources Tabs Fields',
        templateUrl: helper.basepath('hr_tabs_col/hr_tabs_col.html'),
      //  resolve: helper.resolveFor('ngTable')
		 resolve: helper.resolveFor('ngTable','ngDialog')
    })
	.state('app.addHRTabCol', {
        url: '/hr_tabs_col/add',
        title: 'Add Human Resources Tabs Fields',
	...
	.state('app.viewHRTabsCol', {
		url: '/hr_tabs_col/:id/view',
        title: 'View Human Resources Tabs Fields',
	  ...
	  .state('app.editHRTabCol', {
		url: '/hr_tabs_col/:id/edit',
        title: 'Edit Human Resources Tabs Fields',
...
myApp.controller('HRTabsColController', HRTabsColController);
...
function HRTabsColController($scope, $filter, ngParams, $resource, $timeout, ngDataService,$http,ngDialog,toaster,$state) {
    'use strict';
	

	$scope.breadcrumbs = 
		[//{'name':'Dashboard','url':'app.dashboard','isActive':false}, 
		 {'name':'Human Resources Tabs Fields','url':'#','isActive':false}];
		 
    var vm = this;
	 
	var Api = $scope.$root.hr+"hr_tabs_col";
	 ...
	 $scope.tab_id = '';
 	 $scope.showLoader = true;
       ...
       if(tab_id == undefined)
		...
		var colUrl_2 = $scope.$root.hr+"hr_tabs_col/get_column_byid";
	   	   		 $http
...
//  				  columns_data.tab_id = $scope.tabs.id;
	  ...
	  $scope.sorting_tab_fields = function(id,sort_id,str,index,t_id){
    		 var sorUrl = $scope.$root.hr+"hr_tabs_col/sort-tab-col";
	   	    $http 
 ...
 $scope.delete = function (id,index,arr_data) {
    		var delUrl = $scope.$root.hr+"hr_tabs_col/delete-tab-col";
...
function HRTabColAddController($scope, $stateParams, $http, $state,toaster,$timeout){
	...
	$scope.btnCancelUrl = 'app.hr_tabs_col'; 
	$scope.breadcrumbs = 
		[//{'name':'Dashboard','url':'app.dashboard','isActive':false}, 
		 {'name':'Human Resources Tabs Fields','url':'app.hr_tabs_col','isActive':false},
		 {'name':'Add','url':'#', 'isActive':false}];

	$scope.formUrl = function() {
		return "app/views/hr_tabs_col/_form.html";
	  }
	...
	var postUrl = $scope.$root.hr+"hr_tabs_col/add-tab-col";
	var tabUrl = $scope.$root.hr+"hr-tabs/get-all-tabs";
		 ...
		 $http
	      ...
	      .then(function (res) {
				if(res.data.ack == true){
					 ...
					 $timeout(function(){ $state.go('app.hr_tabs_col'); }, 3000);
				}
				else
...
//	$scope.arr_status =	[{'label':'Active','value':1},{'label':'Inactive','value':0}];
	...
	var postUrl = $scope.$root.hr+"hr_tabs_col/get-tab-col";
	var postData = {
	    'token': $scope.$root.token,
	    'id': $stateParams.id
  	};
...
function HRTabColEditController($scope, $stateParams, $http, $state, $resource,toaster,$timeout){
	...
	var postUrlg = $scope.$root.hr+"hr_tabs_col/get-tab-col";
	var postData = {
	    'token': $scope.$root.token,
	    'id': $stateParams.id
  	};
	...
	var postUrl = $scope.$root.hr+"hr_tabs_col/update-tab-col";
	$scope.arr_status =	[{'label':'Active','value':1},{'label':'Inactive','value':0}];
	$scope.rec = {};
	$scope.status = {};
