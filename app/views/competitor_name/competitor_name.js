CompetitorNameController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "toaster", "ngDialog", "$rootScope"];
myApp.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider',
	function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
		/* specific routes here (see file config.js) */
		$stateProvider
			.state('app.competitor_name', {
				url: '/competitor_names',
				title: 'Setup',
				templateUrl: helper.basepath('competitor_name/competitor_name.html'),
				resolve: helper.resolveFor('ngTable', 'ngDialog')
			})
			.state('app.addCompetitorName', {
				url: '/competitor_name/add',
				title: 'Setup',
				templateUrl: helper.basepath('add.html'),
				controller: 'CompetitorNameAddController'
			})
			.state('app.viewCompetitorName', {
				url: '/competitor_name/:id/view',
				title: 'Setup',
				templateUrl: helper.basepath('view.html'),
				resolve: angular.extend(helper.resolveFor('ngDialog'), {
					tpl: function () { return { path: helper.basepath('ngdialog-template.html') }; }
				}),
				controller: 'CompetitorNameViewController'
			})
			.state('app.editCompetitorName', {
				url: '/competitor_name/:id/edit',
				title: 'Setup',
				templateUrl: helper.basepath('edit.html'),
				resolve: angular.extend(helper.resolveFor('ngDialog'), {
					tpl: function () { return { path: helper.basepath('ngdialog-template.html') }; }
				}),
				controller: 'CompetitorNameEditController'
			})

	}]);

myApp.controller('CompetitorNameController', CompetitorNameController);
myApp.controller('CompetitorNameAddController', CompetitorNameAddController);
myApp.controller('CompetitorNameViewController', CompetitorNameViewController);
myApp.controller('CompetitorNameEditController', CompetitorNameEditController);

function CompetitorNameController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, toaster, ngDialog, $rootScope) {
	'use strict';
	...
	$scope.breadcrumbs =
		...
		{ 'name': 'Competitor Names', 'url': '#', 'isActive': false }];
		...
		ngDialog.openConfirm({
			...
			$http
				...
				.then(function (res) {
					if (res.data.ack == true) {
						toaster.pop('success', 'Deleted', $scope.$root.getErrorMessageByCode(103));
						$timeout(function () { $state.go('app.competitor_name'); }, 1500);
					}
					else {
...
function CompetitorNameAddController($scope, $stateParams, $http, $state, toaster, $timeout, $rootScope) {
	//alert("Here");
	$scope.btnCancelUrl = 'app.competitor_name';
	...
	$scope.formUrl = function () {
		return "app/views/competitor_name/_form.html";
	}
		...
		$http
			...
			.then(function (res) {
				if (res.data.ack == true) {
					...
					$timeout(function () { $state.go('app.competitor_name'); }, 1000);
				}
				else
...
function CompetitorNameViewController($scope, $stateParams, $http, $state, $resource, ngDialog, toaster, $timeout, $rootScope) {
	$scope.btnCancelUrl = 'app.competitor_name';
	$scope.breadcrumbs =
		[{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
		{'name': 'Sales', 'url': 'app.setup', 'isActive': false, 'tabIndex':'3'},
		{ 'name': 'Competitor Names', 'url': 'app.competitor_name', 'isActive': false }];
	// $rootScope.currentSetupTab = 6;	 
	$scope.gotoEdit = function () {
		$state.go("app.editCompetitorName", { id: $stateParams.id });
	};
...
function CompetitorNameEditController($scope, $stateParams, $http, $state, $resource, ngDialog, toaster, $timeout, $rootScope) {
