   ...
   $scope.getALLFolder();
    ...
    $scope.get_folder_document = function (id, permisions, item) {
        ...
        $timeout(function () {
            $http
                 .post(Api2, postData)
                 .then(function (res) { $scope.showdoc = false;
                     if (res.data.ack == true) {
...
function editdocController($scope, $resource, $http, toaster, $state, $rootScope, ser1, dataService, $stateParams,$timeout){
...
//        $scope.list_folder[0];
    ...
    $scope.enable_check = function (index, val, arg) {

        if ((arg == 1) || (arg == 5)) {
            $scope.salepersons[index].chk = val;
            $scope.salepersons[index].allowuright_add = val;
            $scope.salepersons[index].allowuright_view = val;
            $scope.salepersons[index].allowuright_edit = val;
            $scope.salepersons[index].allowuright_delete = val;
        }
        else if (arg == 2) {
            $scope.salepersons[index].chk = val;
            $scope.salepersons[index].allowuright_add = val;
            $scope.salepersons[index].allowuright_view = val;
            $scope.salepersons[index].allowuright_edit = val;
            $scope.salepersons[index].allowuright_delete = false;
        }
        else if (arg == 3) {
            $scope.salepersons[index].chk = val;
            $scope.salepersons[index].allowuright_add = val;
            $scope.salepersons[index].allowuright_view = val;
            $scope.salepersons[index].allowuright_edit = false;
            $scope.salepersons[index].allowuright_delete = false;
        }
        else if (arg == 4) {
            $scope.salepersons[index].chk = val;
            $scope.salepersons[index].allowuright_add = val;
            $scope.salepersons[index].allowuright_view = false;
            $scope.salepersons[index].allowuright_edit = false;
            $scope.salepersons[index].allowuright_delete = false;
        }
...
function DocUploadDocument_single($scope, Upload, $timeout, toaster) {
    ...
    $scope.uploadFiles = function (file, errFiles, source) {
        if (errFiles && errFiles.length){
            toaster.pop('error', 'Info', "Invalid file type.");
            return;
        }
        console.log("uploading using doc.js");
        $scope.showLoader = true;
        $('#pic_block').attr("disabled", true);
        //$scope.formData={};
        //$scope.choices={};
        if (source == "employeeProfile" && file != null && file.size > 51200){
            $scope.showLoader = false;
            toaster.pop('error', 'Info', "File size is exceeding the limit of 50Kb.");
...
ImgUploadController.$inject = ["$scope", "$stateParams", "$http", "$state", "$resource", "toaster", "$filter", "$window", "ngDialog", "$timeout", "$rootScope", "Upload" ];
myApp.controller('ImgUploadController', ImgUploadController);
function ImgUploadController($scope, $stateParams, $http, $state, $resource, toaster, $filter, $window, ngDialog, $timeout, $rootScope, Upload ) {
    'use strict';
    $scope.uploadFiles = function (file, errFiles) {
...
//----------- 	 Document 	 ----------------------------------------
    ...
    $scope.getDocuments = function () {return;
        ...
        $http
             .post(API, $scope.postData)
             .then(function (res) {  $scope.showLoader = false;
                 $scope.documentlist = [];
                 $scope.columns = [];
                 if (res.data.ack == true) {
