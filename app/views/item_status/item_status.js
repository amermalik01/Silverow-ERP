...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
 ...
 $stateProvider	  
	.state('app.item-status', {
        url: '/item-status',
        title: 'item Status',
        templateUrl: helper.basepath('item_status/item_status.html'),
        resolve: helper.resolveFor('ngTable','ngDialog')
    })
	.state('app.add-item-status', {
        url: '/item-status/add',
        title: 'Add item Status ',
        templateUrl: helper.basepath('edit.html'),
		controller: 'itemstatusEditController'
    }) 
	  .state('app.edit-item-status', {
		url: '/item-status/:id/edit',
...
ItemstatusController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService","$http","ngDialog","toaster","$rootScope"];
myApp.controller('ItemstatusController', ItemstatusController);
myApp.controller('itemstatusEditController', itemstatusEditController);

function ItemstatusController($scope, $filter, ngParams, $resource, $timeout, ngDataService,$http,ngDialog,toaster,$rootScope) {
    'use strict';
  ...
  //  var Api = $scope.$root.sales+"crm/crm/all-status";
    ...
    var postData = {
	    ...
	    'all': "1",
		'tbl':  $rootScope.base64_encode('product_status') ,
  	};
    ...
    $scope.delete = function (id,rec,arr_data) {
	    ...
	    ngDialog.openConfirm({
	      ...
	      $http
			  .post(delUrl, {id:id,'token': $scope.$root.token,'tbl':$rootScope.base64_encode('product_status')})
			  .then(function (res) {
...
function itemstatusEditController($scope, $stateParams, $http, $state, $resource,toaster,$rootScope,$timeout){

	$scope.formTitle = 'CRM Status';
	 $scope.btnCancelUrl = 'app.item-status';
	 $scope.hideDel = false; 
	...
	$scope.breadcrumbs = 
		[//{'name':'Dashboard','url':'app.dashboard','isActive':false},
		 {'name':'Setup','url':'#','isActive':false},
		 {'name':'Sales','url':'#','isActive':false},
		 {'name':'CRM Status','url':'app.item-status','isActive':false},
	...
	rec.tbl = $rootScope.base64_encode('product_status');
		 ...
		 $http
	      ...
	      .then(function (res) {
				...
				toaster.pop('success', 'Edit', $scope.$root.getErrorMessageByCode(102));
				 $timeout(function(){ $state.go('app.item-status'); }, 3000);
			}
