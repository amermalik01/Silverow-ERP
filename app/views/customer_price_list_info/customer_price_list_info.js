CRMPricelistInfoController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "ngDialog", "toaster", "$stateParams", "$rootScope"];
CRMPricelistInfoAddController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "ngDialog", "toaster", "$stateParams", "$rootScope"];

myApp.controller('CRMPricelistInfoController', CRMPricelistInfoController);
myApp.controller('CRMPricelistInfoAddController', CRMPricelistInfoAddController);


function CRMPricelistInfoController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, ngDialog, toaster, $stateParams, $rootScope) {
    ...
    $scope.$on("myCRMPricelistInfoEventReload", function (event, args) {
       ...
       // $scope.arrCRMIds.push($stateParams.id);
    ...
    $scope.detail = function (id) {
        ...
        $scope.$root.$broadcast("openCRMPricelistInfoFormEvent", {'edit': false, id: id});
    }

    $scope.editForm = function (id) {
        $scope.isVolumeDiscForm = true;
        $scope.isVolumeDiscListing = false;
        $scope.check_volume_readonly = true;
        $scope.viewmode = 0;
        $scope.$root.$broadcast("openCRMPricelistInfoFormEvent", {'edit': true, id: id});
    }

    $scope.$data = {};
    $scope.delete = function (id, index, $data) {
        var delUrl = $scope.$root.sales + "crm/crm/delete-customer-items-info";
...
function CRMPricelistInfoAddController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, ngDialog, toaster, $stateParams, $rootScope) {
    $scope.$root.tabHide = 0;
    ...
    $scope.showCRMPricelistInfoListing = function () {
        $scope.$root.$broadcast("showCRMPricelistInfoListing");
    }

    $scope.showCRMPricelistInfoEditForm = function () {
...
//--------------------------------------------------
    ...
    /* var categoryUrl = $scope.$root.stock + "categories/get-all-categories";
     ...
     .then(function (res) {
     if (res.data.ack == true)    $scope.cat_prodcut_arr = res.data.response;
...
//--------------------------------------------------------------------------------
    ...
    $scope.$on("openCRMPricelistInfoFormEvent", function (event, arg) {
        $scope.arrIds = [];
        ...
        $scope.$root.$broadcast("showCRMPricelistInfoForm");
        var id = arg.id;
        ...
        $timeout(function () {
            ...
            $http
                .post(pOfferUrl, {id: id, 'token': $scope.$root.token})//price_type: $scope.$root.price_type,
                .then(function (res) {
                    ...
                    $timeout(function () {
                        $scope.$root.$apply(function () {
                            ...
                            if (res.data.response.currency_id > 0) var currency_id = res.data.response.currency_id;
                            else  var currency_id = $scope.$root.defaultCurrency;
    ...
    $scope.getOffer = function (isShow, item_paging) {
        ...
        /*angular.element('#price_list_item_popup').modal({show: true});
         return;*/
        ...
        angular.element('#listing_model_emp_info').modal({show: true});
    }
    $scope.confirmOffer = function (result) {
        $scope.rec.offered_by = result.first_name + ' ' + result.last_name;
        $scope.rec.offered_by_id = result.id;
        angular.element('#listing_model_emp_info').modal('hide');
    }
...
//------------------------------------------------------------
    ...
    $scope.add = function (rec) {
        ...
        // console.log(newObj);
        if (rec.id > 0) {
            var addUrl = $scope.$root.sales + "crm/crm/update-customer-item-info";
        }
        ...
        $http
            ...
            .then(function (res) {
                ...
                if (rec.id > 0) {
                    ...
                    //$scope.$root.$broadcast("myCRMPricelistInfoEventReload", args);
    ...
    $scope.add_excluded_customers = function (id, grp_id) {
        ...
        rec.customer_price_list_info_id = id;
        rec.token = $scope.$root.token;
        $http
            .post(excUrl, rec)
            .then(function (res) {
    ...
    $scope.addVolumeDiscount = function (formData) {
        ...
        var updateUrl = $scope.$root.sales + "crm/crm/add-cust-price-list-volume";
        ...
        if ($scope.rec.id > 0 || $scope.formData.id > 0) {
            ...
            formData.customer_price_list_info_id = formData.volume_1.customer_price_list_info_id;
        }
        else {
            ...
            formData.customer_price_list_info_id = formData.volume_1[$scope.count].customer_price_list_info_id;
        }
        ...
        //var updateUrl = $scope.$root.sales + "crm/crm/update-product-values";
            var updateUrl = $scope.$root.sales + "crm/crm/update-cust-price-list-volume";
    ...
    var volumeUrl_item = $scope.$root.sales + "crm/crm/get-sale-cust-price-list-volume";
    var addvolumeUrl_item = $scope.$root.sales + "crm/crm/add-sale-cust-price-list-volume";
    ...
    $scope.add_vol1_type = function (formData_vol_1) {
        ...
        if ($scope.rec.id > 0 || $scope.formData.id > 0) {
            ...
            formData_vol_1.customer_price_list_info_id = $scope.rec.id;
            ...
            formData_vol_1.customer_price_list_info_id = $scope.arrIds[$scope.volCounter];
            formData_vol_1.unit_categorys = $scope.formData_vol_1.unit_category !== undefined ? $scope.formData_vol_1.unit_category.id : 0;
        }
    ...
    $scope.get_item_voume_list = function (id) {
        ...
        /*if ($scope.formData.id != undefined && $scope.formData.id > 0)
            arIds.push($scope.formData.customer_price_list_info_id);
        else
            arIds = $scope.arrIds;*/
        ...
        $scope.volCounter = 0;
        $http
            .post(volumeUrl_item, {isAdded: isAdded, customer_price_list_info_id: arIds, 'token': $scope.$root.token})
...
//---------------------------------------------------------------
    ...
    $scope.addForm = function (rec) {
        $scope.$root.$broadcast("openCRMPricelistInfoFormEvent", {'edit': false});
    }
    ...
    $scope.getproducts = function (isShow, item_paging) {
        ...
        $scope.postData.searchKeyword = $scope.searchKeyword_priceList.searchBox;

        if ($scope.searchKeyword_priceList.category_names != null)
            $scope.postData.category_name = $scope.searchKeyword_price.category_names !== undefined ? $scope.searchKeyword_priceList.category_names.id : 0;
        if ($scope.searchKeyword_priceList.brand_names != null)
            $scope.postData.brand_name = $scope.searchKeyword_priceList.brand_names !== undefined ? $scope.searchKeyword_priceList.brand_names.id : 0;
        if ($scope.searchKeyword_priceList.units != null)
            $scope.postData.unitss = $scope.searchKeyword_priceList.units !== undefined ? $scope.searchKeyword_priceList.units.id : 0;
    ...
    $scope.confirm2 = function (result) {
        ...
        $scope.loadDropDownsData(result.id, restype);
        angular.element('#price_list_item_popup').modal('hide');
    }
