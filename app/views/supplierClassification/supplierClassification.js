supplierClassificationController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "ngDialog", "toaster"];
myApp.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider',
	function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {

		$stateProvider
			.state('app.supplier-classification', {
				url: '/supplier-classification',
				title: 'supplier Classification Setup',
				templateUrl: helper.basepath('supplierClassification/supplierClassification.html'),
				resolve: helper.resolveFor('ngTable', 'ngDialog')
			})
			.state('app.add-supplier-classification', {
				url: '/supplier-classification/add',
				title: 'Add supplier Classification',
				templateUrl: helper.basepath('add.html'),
				controller: 'supplierClassificationAddController'
			})
			.state('app.view-supplier-classification', {
				url: '/supplier-classification/:id/view',
				title: 'View supplier Classification',
				templateUrl: helper.basepath('view.html'),
				controller: 'supplierClassificationAddController'
			})
			.state('app.edit-supplier-classification', {
				url: '/supplier-classification/:id/edit',
				title: 'Edit supplier Classification',
				templateUrl: helper.basepath('edit.html'),
				controller: 'supplierClassificationAddController'
			})
	}]);

myApp.controller('supplierClassificationController', supplierClassificationController);
myApp.controller('supplierClassificationAddController', supplierClassificationAddController);

function supplierClassificationController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, ngDialog, toaster) {
	'use strict';

	$scope.breadcrumbs = [{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
	{ 'name': 'Purchase', 'url': 'app.setup', 'isActive': false, 'tabIndex': '4' },
	{ 'name': 'Supplier Classification', 'url': '#', 'isActive': false }];
...
function supplierClassificationAddController($scope, $stateParams, $http, $state, $resource, toaster, ngDialog, $timeout) {

	$scope.formTitle = 'supplier Classification';
	$scope.btnCancelUrl = 'app.supplier-classification';

	$scope.status = {};
	$scope.arr_status = [{ 'label': 'Active', 'value': 1 }, { 'label': 'Inactive', 'value': 0 }];
	$scope.showStatus = [{ 'label': 'Enable', 'value': 1 }, { 'label': 'Disable', 'value': 0 }];

	$scope.classification_type = [{ 'label': 'SRM', 'value': 3 }, { 'label': 'Supplier', 'value': 4 }];

	$scope.breadcrumbs = [{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
	{ 'name': 'Purchase', 'url': 'app.setup', 'isActive': false, 'tabIndex': '4' },
	{ 'name': 'Supplier Classification', 'url': 'app.supplier-classification', 'isActive': false }];

	$scope.formUrl = function () {
		return "app/views/supplierClassification/_form.html";
	}
	...
	if ($stateParams.id != undefined) {
		...
		var postData = { 'token': $scope.$root.token, 'id': $stateParams.id,'type': 4 };
	...
	$scope.gotoEdit = function () {
		...
		$state.go("app.edit-supplier-classification", { id: $stateParams.id });
	}
			...
			$http
				...
				.then(function (res) {
					if (res.data.ack == true) {
						...
						$timeout(function () {
							$state.go('app.supplier-classification');
						}, 2000);
					}
					else {
		...
		$http
			...
			.then(function (res) {
				if (res.data.ack == true) {
					...
					$scope.$root.updateSelectedGlobalData("supplierclassification");
