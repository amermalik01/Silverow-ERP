...
//        }
        ...
        $scope.rec.account_payable_number = result.alldata.account_payable_number;
        $scope.rec.account_payable_id = result.alldata.account_payable_id;
...
//                             $scope.rec.bill_to_contact_no = $scope.Contprefix + $scope.getQuoteNo(res.data.response.bill_to_contact_id);
                ...
                $scope.$root.currency_id = res.data.response.currency_id.id;
...
function QuoteTabController($scope, $rootScope, $stateParams, $http, $state, $resource, toaster, ngDialog, $timeout, myService) {
    ...
    $scope.getProducts = function (recs, parm, item_paging, clr) {
        ...
        /* $http
         ...
         $scope.showLoader = false;
         */
        $scope.products = $rootScope.prooduct_arr;
...
//------------- Gereral Ledgere -----------------
                                        ...
                                        $http
                                            ...
                                            .then(function (cumData) {
                                                if (cumData.data.ack != true) {
                                                    toaster.pop('error', 'Info', $scope.$root.getErrorMessageByCode(577, [prodData.description]));
                                                    $scope.$root.return_status = false;
                                                    //return false;
                                                }
                                                else {
                                                    ...
                                                    $http
                                                        ...
                                                        .then(function (umData) {
                                                            if (umData.data.ack != true) {
                                                                toaster.pop('error', 'Info', $scope.$root.getErrorMessageByCode(576, [prodData.description]));
                                                                $scope.$root.return_status = false;
                                                                //return false;
                                                            }
                                                            else {
    ...
    $scope.add_sublist = function (rec2) {
        ...
        if ($scope.$root.posting_date == undefined || $scope.$root.posting_date == null) {
            toaster.pop('error', 'info', $scope.$root.getErrorMessageByCode(356, ['Invoice Date']));
            return;
        }
        ...
        console.log($scope.$root.currency_id + $scope.$root.defaultCurrency);

        if ($scope.$root.currency_id == $scope.$root.defaultCurrency) {
    ...
    $scope.$on("quotesDetail", function (event, itemsData) {
        ...
        angular.forEach(itemsData, function (elem, detIndex) {
            ...
            detail.product_code = elem.item_no;
            /*detail.vat = elem.vat;
