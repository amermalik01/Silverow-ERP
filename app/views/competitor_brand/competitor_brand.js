CompetitorBrandController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "toaster", "ngDialog", "$rootScope"];
myApp.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider',
	function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
		/* specific routes here (see file config.js) */
		$stateProvider
			.state('app.competitor_brand', {
				url: '/competitor_brand',
				title: 'Setup',
				templateUrl: helper.basepath('competitor_brand/competitor_brand.html'),
				resolve: helper.resolveFor('ngTable', 'ngDialog')
			})
			.state('app.addCompetitorBrand', {
				url: '/competitor_brand/add',
				title: 'Setup',
				templateUrl: helper.basepath('add.html'),
				controller: 'CompetitorBrandAddController'
			})
			.state('app.viewCompetitorBrand', {
				url: '/competitor_brand/:id/view',
				title: 'Setup',
				templateUrl: helper.basepath('view.html'),
				resolve: angular.extend(helper.resolveFor('ngDialog'), {
					tpl: function () { return { path: helper.basepath('ngdialog-template.html') }; }
				}),
				controller: 'CompetitorBrandViewController'
			})
			.state('app.editCompetitorBrand', {
				url: '/competitor_brand/:id/edit',
				title: 'Setup',
				templateUrl: helper.basepath('edit.html'),
				resolve: angular.extend(helper.resolveFor('ngDialog'), {
					tpl: function () { return { path: helper.basepath('ngdialog-template.html') }; }
				}),
				controller: 'CompetitorBrandEditController'
			})

	}]);

myApp.controller('CompetitorBrandController', CompetitorBrandController);
myApp.controller('CompetitorBrandAddController', CompetitorBrandAddController);
myApp.controller('CompetitorBrandViewController', CompetitorBrandViewController);
myApp.controller('CompetitorBrandEditController', CompetitorBrandEditController);

function CompetitorBrandController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, toaster, ngDialog, $rootScope) {
	'use strict';
	...
	$scope.breadcrumbs =
		...
		{ 'name': 'Competitor Brands', 'url': '#', 'isActive': false }];
		...
		ngDialog.openConfirm({
			...
			$http
				...
				.then(function (res) {
					if (res.data.ack == true) {
						toaster.pop('success', 'Deleted', $scope.$root.getErrorMessageByCode(103));
						$timeout(function () { $state.go('app.competitor_brand'); }, 1500);
					}
					else {
...
function CompetitorBrandAddController($scope, $stateParams, $http, $state, toaster, $timeout, $rootScope) {
	//alert("Here");
	$scope.btnCancelUrl = 'app.competitor_brand';
	...
	$scope.formUrl = function () {
		return "app/views/competitor_brand/_form.html";
	}
		...
		$http
			...
			.then(function (res) {
				if (res.data.ack == true) {
					...
					$timeout(function () { $state.go('app.competitor_brand'); }, 1000);
				}
				else
...
function CompetitorBrandViewController($scope, $stateParams, $http, $state, $resource, ngDialog, toaster, $timeout, $rootScope) {
	$scope.btnCancelUrl = 'app.competitor_brand';
	$scope.breadcrumbs =
		[{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
		{'name': 'Sales', 'url': 'app.setup', 'isActive': false, 'tabIndex':'3'},
		{ 'name': 'Competitor Brands', 'url': 'app.competitor_brand', 'isActive': false }];
	// $rootScope.currentSetupTab = 6;	 
	$scope.gotoEdit = function () {
		$state.go("app.editCompetitorBrand", { id: $stateParams.id });
	};
...
function CompetitorBrandEditController($scope, $stateParams, $http, $state, $resource, ngDialog, toaster, $timeout, $rootScope) {
