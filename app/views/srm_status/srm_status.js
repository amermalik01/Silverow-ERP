...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
 ...
 $stateProvider	  
	.state('app.srm-status', {
        url: '/srm-status',
        title: 'SRM Status',
        templateUrl: helper.basepath('srm_status/srm_status.html'),
        resolve: helper.resolveFor('ngTable','ngDialog')
    })
	.state('app.add-srm-status', {
        url: '/srm-status/add',
        title: 'Add SRM Status ',
        templateUrl: helper.basepath('edit.html'),
		controller: 'SrmStatusEditController'
    }) 
	  .state('app.edit-srm-status', {
		url: '/srm-status/:id/edit',
        title: 'Edit SRM Status ',
        templateUrl: helper.basepath('edit.html'),
		controller: 'SrmStatusEditController'
	  })
...
SmStatusController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService","$http","ngDialog","toaster","$rootScope"];
myApp.controller('SmStatusController', SmStatusController);
myApp.controller('SrmStatusEditController', SrmStatusEditController);

function SmStatusController($scope, $filter, ngParams, $resource, $timeout, ngDataService,$http,ngDialog,toaster,$rootScope) {
    'use strict';
  ...
  //  var Api = $scope.$root.sales+"crm/crm/all-status";
    ...
    var postData = {
	    ...
	    'all': "1",
		'tbl':  $rootScope.base64_encode('srm_status') ,
  	};
    ...
    $scope.delete = function (id,rec,arr_data) {
	    ...
	    ngDialog.openConfirm({
	      ...
	      $http
			  .post(delUrl, {id:id,'token': $scope.$root.token,'tbl':$rootScope.base64_encode('srm_status')})
			  .then(function (res) {
...
function SrmStatusEditController($scope, $stateParams, $http, $state, $resource,toaster,$rootScope,$timeout){

	$scope.formTitle = 'CRM Status';
	 $scope.btnCancelUrl = 'app.srm-status';
	 $scope.hideDel = false; 
	...
	$scope.breadcrumbs = 
		[//{'name':'Dashboard','url':'app.dashboard','isActive':false},
		 {'name':'Setup','url':'#','isActive':false},
		 {'name':'Sales','url':'#','isActive':false},
		 {'name':'CRM Status','url':'app.srm-status','isActive':false},
	...
	rec.tbl = $rootScope.base64_encode('srm_status');
		 ...
		 $http
	      ...
	      .then(function (res) {
				...
				toaster.pop('success', 'Edit', $scope.$root.getErrorMessageByCode(102));
				 $timeout(function(){ $state.go('app.srm-status'); }, 3000);
			}
