ConfirmationLink.$inject = ["$scope", "$filter", "$resource", "$timeout", "ngTableDataService", "$http", "toaster", "ngDialog", "$rootScope", "$stateParams", "$state"];

myApp.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider',
	function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
		/* specific routes here (see file config.js) */
		$stateProvider
			.state('app.viewConfirmationLink', {
				url: '/confirmation_link/:id/:key',
				title: 'Confirm Action',
				templateUrl: helper.basepath('confirmation_link/confirmation_link.html'),
				resolve: angular.extend(helper.resolveFor('ngDialog'), {
					tpl: function () { return { path: helper.basepath('ngdialog-template.html') }; }
				}),
				controller: 'ConfirmationLink'
			})

	}]);

myApp.controller('ConfirmationLink', ConfirmationLink);

function ConfirmationLink($scope, $filter, $resource, $timeout, ngDataService, $http, toaster, ngDialog, $rootScope, $stateParams, $state) {
	'use strict';
	$scope.showLoader = true;
	console.log($stateParams);

	var actionUrl = $scope.$root.setup + "general/performConfirmation";

	$http
		.post(actionUrl, { id: $stateParams.id, key: $stateParams.key })
		.then(function (res) {
			if (res.data.ack == true) {
				...
				$scope.confirmationMessage = res.data.message;
