CustomerItemInfoController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService","$http","ngDialog","toaster","$stateParams"];
CustomerItemInfoAddController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService","$http","ngDialog","toaster","$stateParams","$rootScope"];

myApp.controller('CustomerItemInfoController', CustomerItemInfoController);
myApp.controller('CustomerItemInfoAddController', CustomerItemInfoAddController);


function CustomerItemInfoController($scope, $filter, ngParams, $resource, $timeout, ngDataService,$http,ngDialog,toaster, $stateParams) {
    'use strict';
	...
	if($scope.$root.crm_id > 0)
		$scope.postData = {'column':'product_id','value':$stateParams.id,token:$scope.$root.token,'more_fields':$scope.more_fields}
	...
	$scope.$on("myCustItemInfoEventReload", function (event, args) {
		...
		if(args != undefined){
			$scope.postData = {'crm_ids':$scope.arrCRMIds,token:$scope.$root.token,'more_fields':$scope.more_fields}
	...
	$scope.search_data = '';
  	$scope.getPricing = function(parm) {			
  		if(parm != undefined){	
			...
			$scope.postData = {'crm_ids':$scope.arrCRMIds,'type_id':type_id,'search_data':$scope.search_data,token:$scope.$root.token,'more_fields':$scope.more_fields}
		}
...
$scope.detail = function(id){
	...
	$scope.getGroupOfCustomers = function(rec){
		...
		$scope.columns = [];
		$scope.arr_customers = [];
		var postData = {type:rec.type,customer_item_info_id:rec.id,price_offered:rec.price_offered,sale_price:$scope.sale_price,'token': $scope.$root.token,'all': "1"};
	...
	$scope.getProductsCustomers = function(rec){
		...
		var postData = {customer_item_info_id:rec.id,customer_product_type_id:rec.customer_product_type_id,product_id:rec.product_id,price_strategy_type:$scope.priceStrategy[1].id,price_strategy:$scope.priceStrategy[2].id,discount_type:$scope.lstPromotion.discount_type,discount:$scope.lstPromotion.discount,'token': $scope.$root.token,'all': "1"};
...
function CustomerItemInfoAddController($scope, $filter, ngParams, $resource, $timeout, ngDataService,$http,ngDialog,toaster, $stateParams,$rootScope){
	$scope.$root.tabHide = 0;
	...
	$scope.search_data = '';
	$scope.cust_types = 0;
	$scope.time_types = [{'label': 'Hours', 'value': 1},{'label': 'Days', 'value': 2}, {'label': 'Weeks', 'value': 3}, {'label': 'Months', 'value': 4}];
	...
	var constUrl = $scope.$root.setup+"ledger-group/get-predefine-by-type";
	 ...
	 $scope.showCustItemInfoListing = function(){
		 ...
		 args[1] = undefined;
		 $scope.$root.$broadcast("myCustItemInfoEventReload", args);*/
	  }
				...
				$timeout(function(){
					$scope.$root.$apply(function(){
						...
						$.each($scope.arr_unit_of_measure,function(index,elem){
							if(elem.id == res.data.response.unit_of_measure_id){$scope.rec.unit_of_measures = elem;}
						});
				        		...
				        		if(res.data.response.customer_product_type_id == 2){
										$.each($scope.arr_customers,function(index,elem){
											if(elem.id == res.data.response.crm_id){
												$scope.rec.customer_name = elem.Name;
      ...
      .then(function (res) {
           ...
           if(res.data.response.length > 0 && $scope.rec.id > 0){
	        		if($scope.rec.cust_prod_type.id > 2){
				  	...
				  	$timeout(function(){$scope.selectedCustomers;},3000);
			}
      });
 ...
 $scope.arrIds = [];
  $scope.add = function(rec){
  	if(rec.cust_prod_type.id == 1){
	 	 if($scope.selectedCustomers.length == 0){
				 toaster.pop('error', 'Info', $scope.$root.getErrorMessageByCode(230, ['Cuatomer(s)']));
	 	 		return false;
	 	 	}
	 }
