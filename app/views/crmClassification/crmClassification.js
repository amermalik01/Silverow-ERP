crmClassificationController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "ngDialog", "toaster"];
myApp.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider',
	function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {

		$stateProvider
			
			.state('app.crm-customer-classification', {
				url: '/crm-classification',
				title: 'Setup',
				templateUrl: helper.basepath('crmClassification/crm_customer_Classification.html'),
				resolve: helper.resolveFor('ngTable', 'ngDialog')
			})			
			.state('app.add-crm-classification', {
				url: '/crm-classification/add',
				title: 'Setup',
				templateUrl: helper.basepath('add.html'),
				controller: 'crmClassificationAddController'
			})
			.state('app.view-crm-classification', {
				url: '/crm-classification/:id/view',
				title: 'Setup',
				templateUrl: helper.basepath('view.html'),
				controller: 'crmClassificationAddController'
			})
			.state('app.edit-crm-classification', {
				url: '/crm-classification/:id/edit',
				title: 'Setup',
	...
	/* .state('app.crm-classification', {
				url: '/crm-classification',
				title: 'CRM Classification Setup',
				templateUrl: helper.basepath('crmClassification/crmClassification.html'),
				resolve: helper.resolveFor('ngTable', 'ngDialog')
			}) */
myApp.controller('crmClassificationController', crmClassificationController);
myApp.controller('crmClassificationAddController', crmClassificationAddController);

function crmClassificationController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, ngDialog, toaster) {
	'use strict';
...
function crmClassificationAddController($scope, $stateParams, $http, $state, $resource, toaster, ngDialog, $timeout) {

	$scope.formTitle = 'CRM Classification';
	$scope.btnCancelUrl = 'app.crm-classification';

	$scope.status = {};
	$scope.arr_status = [{ 'label': 'Active', 'value': 1 }, { 'label': 'Inactive', 'value': 0 }];
	$scope.showStatus = [{ 'label': 'Enable', 'value': 1 }, { 'label': 'Disable', 'value': 0 }];

	$scope.classification_type = [{ 'label': 'crm', 'value': 3 }, { 'label': 'crm', 'value': 4 }];

	$scope.breadcrumbs = [{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
	{ 'name': 'Sales', 'url': 'app.setup', 'isActive': false, 'tabIndex': '3' },
	{ 'name': 'CRM Classification', 'url': 'app.crm-classification', 'isActive': false }];

	$scope.formUrl = function () {
		return "app/views/crmClassification/_form.html";
	}
	...
	if ($stateParams.id != undefined) {
		...
		var postData = { 'token': $scope.$root.token, 'id': $stateParams.id,'type': 1 };
	...
	$scope.gotoEdit = function () {
		...
		$state.go("app.edit-crm-classification", { id: $stateParams.id });
	}
			...
			$http
				...
				.then(function (res) {
					if (res.data.ack == true) {
						...
						$timeout(function () {
							$state.go('app.crm-classification');
						}, 1000);
					}
					else {
		...
		$http
			...
			.then(function (res) {
				if (res.data.ack == true) {
					...
					$scope.$root.updateSelectedGlobalData("crmclassification");
