CrmPriceOfferListingController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "ngDialog", "toaster"];

myApp.controller('CrmPriceOfferListingController', CrmPriceOfferListingController);
myApp.controller('CrmPriceOfferListingAddController', CrmPriceOfferListingAddController);


function CrmPriceOfferListingController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, ngDialog, toaster, $stateParams) {
	'use strict';

	$scope.module_id = 74;
	$scope.filter_id = 119;
	$scope.module_table = 'crm_price_offer';
	$scope.more_fields = 'volume_1_price*volume_2_price*volume_3_price';
	$scope.condition = 0;
	...
	$scope.$on("myCrmPriceOfferListingEventReload", function (event, args) {
	...
	$scope.detail = function (id) {
		...
		$scope.$root.$broadcast("openCrmPriceOfferListingFormEvent", { 'edit': false, id: id });
	}

	$scope.editForm = function (id) {
		$scope.$root.$broadcast("openCrmPriceOfferListingFormEvent", { 'edit': true, id: id });
	}
	...
	$scope.delete = function (id, index, $data) {
		var delUrl = $scope.$root.sales + "crm/crm/delete-crm-price-offer-listing";
...
function CrmPriceOfferListingAddController($scope, $stateParams, $http, $state, $resource, toaster, $timeout, ngDialog, $rootScope, $filter) {
	$scope.$root.tabHide = 0;
	...
	$scope.showCrmPriceOfferListingListing = function () {
		$scope.$root.$broadcast("showCrmPriceOfferListingListing");
	}

	$scope.showCrmPriceOfferListingEditForm = function () {
	...
	$scope.$on("showAddPOfferListingForm", function () {
		$scope.check_readonly = false;
		$scope.showItms = true;
		$scope.showServ = true;
		$scope.resetForm();
		...
		if ($rootScope.defaultDateFormat == $rootScope.dtYMD)
			$scope.listed_date = year + "/" + month + "/" + day;
		if ($rootScope.defaultDateFormat == $rootScope.dtMDY)
			$scope.listed_date = month + "/" + day + "/" + year;
		if ($rootScope.defaultDateFormat == $rootScope.dtDMY)
			$scope.listed_date = day + "/" + month + "/" + year;

		$scope.list_date = $filter('date')($scope.listed_date, $scope.$root.dateFormats[$scope.$root.defaultDateFormat]);
		$scope.rec.offer_date = $filter('date')($scope.listed_date, $scope.$root.dateFormats[$scope.$root.defaultDateFormat]);
	...
	$scope.$on("openCrmPriceOfferListingFormEvent", function (event, arg) {
		$http
			.post(constUrl, { 'token': $scope.$root.token, type: 'OFFER_METHOD' })
			.then(function (res) {
				if (res.data.ack == true) {
		...
		$timeout(function () {
			...
			$scope.$root.$broadcast("showCrmPriceOfferListingForm");
			/*$scope.CrmPriceOfferListingFormShow = false;
				$scope.CrmPriceOfferListingListingShow = true;*/
			var CrmPriceOfferListingUrl = $scope.$root.sales + "crm/crm/get-crm-price-offer-listing";

			$scope.rec = {};
			var table = 'crm_price_offer';
			$http
				.post(CrmPriceOfferListingUrl, { id: id, 'token': $scope.$root.token })
				.then(function (res) {
					$scope.loadDropDownsData(res.data.response.product_id, res.data.response.type);
					$scope.rec = res.data.response;
					$scope.rec.id = res.data.response.id;
					$scope.list_date = $filter('date')(res.data.response.created_date, $scope.$root.dateFormats[$scope.$root.defaultDateFormat]);
	...
	$scope.getProduct = function () {
		$timeout(function () {
			if ($scope.rec.id != undefined) {
				if ($scope.rec.type == 1)
					angular.element("#pOfferListItems a").click();
				if ($scope.rec.type == 2)
					angular.element("#pOfferListServices a").click();
			}
			else
				angular.element("#pOfferListItems a").click();
		}, 2000);
		angular.element('#itemServiceModal').modal({ show: true });
	}
	...
	$scope.addItem = function (item, type) {
		...
		angular.element('#itemServiceModal').modal('hide');
	}
	...
	$scope.add = function (rec) {
  	...
  	return;*/
		...
		if (rec.id != undefined)
			addUrl = $scope.$root.sales + "crm/crm/update-crm-price-offer-listing";
	...
	$scope.onChange_vol_1 = function (arg_id, arg_name) {
		...
		if (arg_name == 'sale') {
			if (id == -1) $('#add_volume').modal({ show: true });
		}
	...
	$scope.add_vol1_type = function (formData_vol_1, volume) {
