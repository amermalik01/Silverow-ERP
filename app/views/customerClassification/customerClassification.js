customerClassificationController.$inject = ["$scope", "$filter", "ngTableParams", "$resource", "$timeout", "ngTableDataService", "$http", "ngDialog", "toaster"];
myApp.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider',
	function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {

		$stateProvider
			.state('app.customer-classification', {
				url: '/customer-classification',
				title: 'customer Classification Setup',
				templateUrl: helper.basepath('customerClassification/customerClassification.html'),
				resolve: helper.resolveFor('ngTable', 'ngDialog')
			})
			.state('app.add-customer-classification', {
				url: '/customer-classification/add',
				title: 'Add customer Classification',
				templateUrl: helper.basepath('add.html'),
				controller: 'customerClassificationAddController'
			})
			.state('app.view-customer-classification', {
				url: '/customer-classification/:id/view',
				title: 'View customer Classification',
				templateUrl: helper.basepath('view.html'),
				controller: 'customerClassificationAddController'
			})
			.state('app.edit-customer-classification', {
				url: '/customer-classification/:id/edit',
				title: 'Edit customer Classification',
				templateUrl: helper.basepath('edit.html'),
				controller: 'customerClassificationAddController'
			})
	}]);

myApp.controller('customerClassificationController', customerClassificationController);
myApp.controller('customerClassificationAddController', customerClassificationAddController);

function customerClassificationController($scope, $filter, ngParams, $resource, $timeout, ngDataService, $http, ngDialog, toaster) {
	'use strict';

	$scope.breadcrumbs = [{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
	{ 'name': 'Sales', 'url': 'app.setup', 'isActive': false, 'tabIndex': '3' },
	{ 'name': 'Customer Classification', 'url': '#', 'isActive': false }];
...
function customerClassificationAddController($scope, $stateParams, $http, $state, $resource, toaster, ngDialog, $timeout) {

	$scope.formTitle = 'customer Classification';
	$scope.btnCancelUrl = 'app.customer-classification';

	$scope.status = {};
	$scope.arr_status = [{ 'label': 'Active', 'value': 1 }, { 'label': 'Inactive', 'value': 0 }];
	$scope.showStatus = [{ 'label': 'Enable', 'value': 1 }, { 'label': 'Disable', 'value': 0 }];

	$scope.classification_type = [{ 'label': 'customer', 'value': 3 }, { 'label': 'customer', 'value': 4 }];

	$scope.breadcrumbs = [{ 'name': 'Setup', 'url': 'app.setup', 'isActive': false, 'tabIndex': '1' },
	{ 'name': 'Sales', 'url': 'app.setup', 'isActive': false, 'tabIndex': '3' },
	{ 'name': 'Customer Classification', 'url': 'app.customer-classification', 'isActive': false }];

	$scope.formUrl = function () {
		return "app/views/customerClassification/_form.html";
	}
	...
	if ($stateParams.id != undefined) {
		...
		var postData = { 'token': $scope.$root.token, 'id': $stateParams.id,'type': 2 };
	...
	$scope.gotoEdit = function () {
		...
		$state.go("app.edit-customer-classification", { id: $stateParams.id });
	}
			...
			$http
				...
				.then(function (res) {
					if (res.data.ack == true) {
						...
						$timeout(function () {
							$state.go('app.customer-classification');
						}, 1000);
					}
					else {
		...
		$http
			...
			.then(function (res) {
				if (res.data.ack == true) {
					...
					$scope.$root.updateSelectedGlobalData("customerclassification");
