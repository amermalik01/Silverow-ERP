...
<<<<<<< HEAD
		...
		left JOIN departments  on departments.id=es.department 
		where 
		(es.user_company=" . $this->arrUser['company_id'] . " or  company.parent_id=" . $this->arrUser['company_id'] . ")
		AND  es.id in (" . $strIds . ")
		ORDER BY es.id DESC
		";
=======
...
/* commented ". $this->objGeneral->current_stock_counter($this->arrUser['company_id']) ." from below query */
	...
	where product.product_code IS NOT NULL
...
<<<<<<< HEAD
        ...
        $Sql = "SELECT cb.* FROM  bucket_filters  cb 
	left JOIN company on company.id=cb.company_id 
	WHERE cb.bucket_id = ".$arr_attr['bucket_id']."  AND cb.type = 1 
	AND (cb.company_id=" . $this->arrUser['company_id'] . " or  company.parent_id=" . $this->arrUser['company_id'] . ")	
	";
//echo $Sql;exit;
=======
...
<<<<<<< HEAD
            ...
            $Sql = "SELECT  c.id,c.crm_code,c.region,c.statusp,c.segment,c.buying_group,c.name,
            c.primaryc_name,c.primary_city,c.primary_county,c.primary_country,c.primary_postcode,c.phone,c.type,c.salesperson_name, c.turnover
            from sr_crm_listing  c 
            where ( c.type IN (1) AND c.crm_code IS NOT NULL AND  c.name !='' AND
...
<<<<<<< HEAD
    function createSQLQueryForFilter($attr){
        ...
        $new = '';
        $where_clause_loc = '';
        if (!empty($attr[array_dynamic_filter_product]) && !empty($attr[array_dynamic_filter_product][0]->normal_filter->field_name) && !empty($attr[array_dynamic_filter_product][0]->operator_filter->id) && (!empty($attr[array_dynamic_filter_product][0]->operator_search) || $attr[array_dynamic_filter_product][0]->operator_search == 0)
        ) {
