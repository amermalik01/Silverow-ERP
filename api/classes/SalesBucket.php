...
class SalesBucket extends Xtreme
{
	...
	function get_customer_crm_by_sale_person_id($attr)
	{
		...
		$where_clause = '';
		if (
			!empty($attr['array_dynamic_filter']) && !empty($attr['array_dynamic_filter'][0]->normal_filter->id)
	...
	function get_sale_customer_data($arr_attr)
	{
		...
		$Sql = "SELECT cb.* 
				...
				WHERE cb.module_id = '" . $arr_attr['module_id'] . "'  AND cb.type = 1 AND 
				cb.company_id=" . $this->arrUser['company_id'] . " ";
		//Order by cb.id DESC 
	...
	function add_sale_customer_data($attr)
	{
		...
		if ($chk > 0) {
			...
			$query_format = "SELECT  distinct c.name,c.type, c.id, c.crm_no, c.customer_code, c.crm_code, c.customer_no, c.name , c.city  , c.postcode, c.turnover , cn.name as cnname , cr.title as region  , bs.title as buying_group  , sr.title as segment 
							 FROM  crm  c 	
							 left JOIN country as cn on cn.id=c.country_id
							 left JOIN crm_region as cr on cr.id =c.region_id 
							 left JOIN crm_buying_group as bs on bs.id =c.buying_grp 
							 left JOIN crm_segment as sr on sr.id =c.company_type
							 WHERE c.name!='' AND  c.type IN (2,3) AND c.company_id=" . $this->arrUser['company_id'] . " " . $where_clause . " 
							 Order by c.id DESC	";
	...
	function update_crm_salesperson($arr_attr)
	{
		...
		//replae in crm
		$updatecrmsql = "UPDATE crm_salesperson SET  salesperson_id='" . $arr_attr['replace_sale_person'] . "',is_primary='$arr_attr[primary_sale_id]' 
	...
	function get_sale_person_baket_list($attr)
	{
		...
		$Sql = "SELECT   c.* 
				FROM  crm_sale_bucket_sp c
				where  c.status=1 AND c.company_id=" . $this->arrUser['company_id'] . " $where_clause
				order by c.id DESC";
