...
$app->post('/sales/customer/return-order/update-credit-note-invoice', function () use ($app) {
    global $objCustomer, $input;
    $input_array = array();
    foreach ($input as $key => $val) {
        $input_array[$key] = $val;
    }
    $result = $objCustomer->updateCreditNoteInvoice($input_array);
    $app->response->setStatus(200);
    $app->response()->headers->set('Content-Type', 'application/json');
    echo json_encode($result);
});
...
$app->post('/sales/customer/order/update-posted-sale-invoice', function () use ($app) {
    global $objCustomer, $input;
    $input_array = array();
    foreach ($input as $key => $val) {
        $input_array[$key] = $val;
    }
    $result = $objCustomer->update_posted_sale_invoice($input_array);
    $app->response->setStatus(200);
    $app->response()->headers->set('Content-Type', 'application/json');
    echo json_encode($result);
});
...
$app->post('/sales/customer/order/chk-order-payment', function () use ($app) {	
	global $objCustomer, $input;
    $input_array = array();
    foreach($input as $key => $val){
        $input_array[$key] = $val;
    }
	$result = $objCustomer->chkOrderPayment($input_array);
	$app->response->setStatus(200);
	$app->response()->headers->set('Content-Type', 'application/json');
	echo json_encode($result);
});

$app->post('/sales/customer/order/get-customer-journal-for-invoice', function () use ($app) {	
	global $objCustomer, $input;
    $input_array = array();
    foreach($input as $key => $val){
        $input_array[$key] = $val;
    }
	$result = $objCustomer->getCusJournalInvoice($input_array);
	$app->response->setStatus(200);
	$app->response()->headers->set('Content-Type', 'application/json');
	echo json_encode($result);
});

$app->post('/sales/customer/order/add-customer-journal-for-invoice', function () use ($app) {	
	global $objCustomer, $input;
    $input_array = array();
    foreach($input as $key => $val){
        $input_array[$key] = $val;
    }
	$result = $objCustomer->addCusJournalInvoice($input_array);
	$app->response->setStatus(200);
	$app->response()->headers->set('Content-Type', 'application/json');
	echo json_encode($result);
});

$app->post('/sales/customer/order/update-customer-journal-for-invoice', function () use ($app) {	
	global $objCustomer, $input;
    $input_array = array();
    foreach($input as $key => $val){
        $input_array[$key] = $val;
    }
	$result = $objCustomer->updateCusJournalInvoice($input_array);
	$app->response->setStatus(200);
	$app->response()->headers->set('Content-Type', 'application/json');
	echo json_encode($result);
});
